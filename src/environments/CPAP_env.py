...
class CpapEnv(BaseSetPointEnv):
    ...
    def simulation_model(self,
                         ...
                         tau_f,                      # Filter constant          # Generated by ensemble
                         dt, f_s,                    # Discretization constants # Generated by ensemble
                        ) -> dict[str, float]:
        """ 
        u_t -> action [V] \\
        current_flow [ml / s], \\
        ...
        self.control_filtered = self.control_filtered + dt / tau_f * (u_t - self.control_filtered)

 
        lung = CpapEnv.Lung(r_aw, c_rs)
        ventilator = CpapEnv.Ventilator(v_t, self.control_filtered, kv, tv, self.phase_counter * dt, rr, t_i, t_ip)
        
        # Filtro de passa baixa (low-pass)
        # self.control_filtered = self.control_filtered + dt / tau_f * (ventilator.peep - self.control_filtered)
        ...
        if self.phase == 'exhale':
            ...
            inhale_noise = np.random.normal(0, inhale_sigma)
            current_flow = ventilator._f_i + inhale_noise
